...
static float pointSize = 0.0; // Point sprite size.
...
void drawScene(void)
{
	...
	for (int i = 0; i < 6; i++)
	{
		drawPoint((i % 2) ? 90.0 - pointSize : pointSize, (width <= height) ? width / 40.0 : height / 40.0, angle + (float)i * PI / 3.0);
	}
...
void animate(int someValue)
{
	if (isAnimate)
	{
		(pointSize >= 60.0) ? pointSize = 20.0 : pointSize += 0.5;
