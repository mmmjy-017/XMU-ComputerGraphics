////////////////////////////////////////////////////////////////////////////////////         
// spaceTravelFrustumCulledTimerQuery.cpp
//
// This program is based on spaceTravelFrustumCulled.cpp with added timer queries to
// output to the C++ window the total time spent drawing asteroids.
// 
...
static unsigned int query[2]; // Query ids. 
static unsigned int resultAvailable[2] = {0, 0}; // Is the query result available?
static unsigned long long timeLeftViewport = 0; // Time drawing left viewport.
static unsigned long long timeRightViewport = 0; // Time drawing right viewport.
...
private:
   ...
   std::list<Asteroid> asteroidList; // Local list of asteroids intersecting the square - only filled for leaf nodes.
   ...
   float x3, float z3, float x4, float z4)
{
	  ...
	  SWCornerX + size, SWCornerZ - size, SWCornerX + size, SWCornerZ))
   {
	  if (SWChild == NULL) // Square is leaf.
	  {
		 ...
		 for (auto asteroid : asteroidList) { asteroid.draw(); }
	  }
	  else
	  {
...
void setup(void)
{
   ...
   glGenQueries(2, query); // Generate 2 query objects, putting ids in query.
}
...
void drawScene(void)
{
   ...
   // Begin timer query.
   glBeginQuery(GL_TIME_ELAPSED, query[0]); // Begin timer query
	  ...
	  // Draw all the asteroids in arrayAsteroids.
   {
	  for (j = 0; j<COLUMNS; j++)
		...
		// along the spacecraft's axis. Note that the tip is at 
		// ( xVal - 10 * sin( (PI/180.0) * angle), 0, zVal - 10 * cos( (PI/180.0) * angle) ).
	  asteroidsQuadtree.drawAsteroids(xVal - 10 * sin((PI / 180.0) * angle) - 7.072 * sin((PI / 180.0) * (45.0 + angle)),
		 zVal - 10 * cos((PI / 180.0) * angle) - 7.072 * cos((PI / 180.0) * (45.0 + angle)),
		 xVal - 10 * sin((PI / 180.0) * angle) - 353.6 * sin((PI / 180.0) * (45.0 + angle)),
		 zVal - 10 * cos((PI / 180.0) * angle) - 353.6 * cos((PI / 180.0) * (45.0 + angle)),
		 xVal - 10 * sin((PI / 180.0) * angle) + 353.6 * sin((PI / 180.0) * (45.0 - angle)),
		 zVal - 10 * cos((PI / 180.0) * angle) - 353.6 * cos((PI / 180.0) * (45.0 - angle)),
		 xVal - 10 * sin((PI / 180.0) * angle) + 7.072 * sin((PI / 180.0) * (45.0 - angle)),
		 zVal - 10 * cos((PI / 180.0) * angle) - 7.072 * cos((PI / 180.0) * (45.0 - angle))
	  );
   ...
   std::cout << "Total time drawing asteroids: " << (timeLeftViewport + timeRightViewport) / 1000000 << " msecs." << std::endl;
...
{
   std::cout << "ALERT: The OpenGL window may take a while to come up because" << std::endl
	  << "of the time to build the quadtree!" << std::endl
	  << std::endl;
   ...
   std::cout << "Press the left/right arrow keys to turn the craft." << std::endl
	  ...
	  << "Press space to toggle between frustum culling enabled and disabled." << std::endl;
}
...
int main(int argc, char **argv)
{
   ...
   glutCreateWindow("spaceTravelFrustumCulledTimerQuery.cpp");
