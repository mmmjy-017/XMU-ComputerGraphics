...
//
// Modification of litTexturedCylinderShaderized.cpp replacing the can
// label texture with a procedural texture, plus an option to discard
// rectangular pieces of the can.
//
//
// Interaction:
// Press x, X, y, Y, z, Z to turn the cylinder.
// Press space to toggle between discarding pieces of the can and not.
//
// Sumanta Guha
//
...
static unsigned int renderHoles = 0; // Render can with holes?
...
void setup(void)
{
	...
	// Obtain renderHoles uniform location.
	renderHolesLoc = glGetUniformLocation(programId, "renderHoles");
...
void drawScene(void)
{
	...
	glUniform1ui(renderHolesLoc, renderHoles); // Update if to render with holes. 
...
void printInteraction(void)
{
	...
	std::cout << "Press x, X, y, Y, z, Z to turn the cylinder." << std::endl
		<< "Press space to toggle between discarding pieces of the can and not." << std::endl;
}
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("proceduralTexture.cpp");
