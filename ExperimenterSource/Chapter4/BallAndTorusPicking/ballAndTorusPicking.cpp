...
void pickFunction(int button, int state, int x, int y)
{
	...
	// Define a selection volume corresponding to selecting in 3 x 3 region around the cursor.
	glLoadIdentity();
...
void printInteraction(void)
{
	...
	std::cout << "Press space to toggle between animation on and off." << std::endl
		...
		<< "Click left mouse button in OpenGL window to select either ball or torus." << std::endl;
}
