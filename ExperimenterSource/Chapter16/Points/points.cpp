...
// 
// Interaction:
// Press right/left arrow keys to cycle through four different point renderings.
...
static vec4 pointColor = vec4(1.0, 0.0, 0.0, 1.0);
...
void setup(void)
{
   ...
   pointColorLoc = glGetUniformLocation(programId, "pointColor");
   glUniform4fv(pointColorLoc, 1, &pointColor[0]);
...
void specialKeyInput(int key, int x, int y)
{
   ...
   {
	  renderChoice = (renderChoice == 0) ? 3 : --renderChoice;
   }
   if (key == GLUT_KEY_UP)
   {
      zTrans += 0.05;
...
void printInteraction(void)
{
   ...
   std::cout << "Press right/left arrow keys to cycle through four different point renderings." << std::endl
	    << "Press up/down arrow keys to move points in the z-direction." << std::endl;
}
