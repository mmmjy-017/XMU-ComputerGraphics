...
// demonstrating use of shader subroutines to switch control between drawing
// sphere and torus.
//
// Interaction:
...
void setup(void)
{
	...
	sphSubroutineIndex = glGetSubroutineIndex(programId, GL_VERTEX_SHADER, "sphere");
...
void drawScene(void)
{
	...
	// Draw torus.
	glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &torSubroutineIndex); // Invoke torus subroutine.
	...
	// Draw ball.
	glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &sphSubroutineIndex); // Invoke sphere subroutine.
	glUniformMatrix4fv(modelViewMatLoc, 1, GL_FALSE, value_ptr(modelViewMat)); // Update modelview matrix.
	glBindVertexArray(vao[SPHERE]);
