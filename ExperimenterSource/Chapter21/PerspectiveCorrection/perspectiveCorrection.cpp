...
void writeData(void)
{
	...
	writeBitmapString((void*)font, "Fraction of the distance the point is");
	glRasterPos3f(-1.1, 1.0, -2.0);
	writeBitmapString((void*)font, "from left end to right: ");
	writeBitmapString((void*)font, theStringBuffer);

	floatToString(theStringBuffer, 5, u);
	glRasterPos3f(-1.1, 0.85, -2.0);
	writeBitmapString((void*)font, "Fraction of the way the point's color is");
	glRasterPos3f(-1.1, 0.8, -2.0);
	writeBitmapString((void*)font, "from red to green:   ");
	writeBitmapString((void*)font, theStringBuffer);
}
...
void printInteraction(void)
{
	...
	std::cout << "Press the left/right arrow keys to move the point." << std::endl
		      << "Press the up/down arrow keys to change its color." << std::endl;
}
